...
import dao.face.AdminQnaDao;
import dao.impl.AdminQnaDaoImpl;
...
public class AdminQnaServiceImpl implements AdminQnaService {
	...
	private AdminQnaDao adminQnaDao = new AdminQnaDaoImpl();

	@Override
	public List<Qna> getList() {

		// 게시글 전체 조회 결과 반환
		return adminQnaDao.selectAll(JDBCTemplate.getConnection());
	...
	public List<Qna> getList(Paging paging) {
		...
		// 페이징 적용해서 조회 결과 반환
		return adminQnaDao.selectAll(JDBCTemplate.getConnection(), paging);
	...
	public Paging getPaging(HttpServletRequest req) {
		...
		// 총 게시글 수 조회하기
		int totalCount = adminQnaDao.selectCntAll(JDBCTemplate.getConnection());
	...
	public Qna view(Qna qnano) {
		...
		// 조회수 증가
		if (adminQnaDao.updateHit(conn, qnano) > 0) {
		...
		// 게시글 조회
		Qna qna = adminQnaDao.selectBoardByBoardno(conn, qnano);
	...
	public void write(HttpServletRequest req) {
		...
		// 게시글 번호 생성
		int qnano = adminQnaDao.selectBoardno(conn);
...
//				qna.setUserno( (int) req.getSession().getAttribute("userno") );
		...
		if (boardFile.getFilesize() != 0) {
			...
			if (adminQnaDao.insertFile(conn, boardFile) > 0) {
				JDBCTemplate.commit(conn);
			} else {
				JDBCTemplate.rollback(conn);
			}
	...
	public String getNick(Qna viewQna) {
		return adminQnaDao.selectNickByUserid(JDBCTemplate.getConnection(), viewQna);
	}

	@Override
	public BoardFile viewFile(Qna viewQna) {
		return adminQnaDao.selectFile(JDBCTemplate.getConnection(), viewQna);
	...
	public void update(HttpServletRequest req) {
		...
		if (adminQnaDao.update(conn, qna) > 0) {
			JDBCTemplate.commit(conn);
		} else {
			JDBCTemplate.rollback(conn);
		}
	...
	public void delete(Qna qna) {
		...
		// 첨부 파일 삭제
		if (adminQnaDao.deleteFile(conn, qna) > 0) {
			JDBCTemplate.commit(conn);
		} else {
			JDBCTemplate.rollback(conn);
		}
		// 글 삭제
		if (adminQnaDao.delete(conn, qna) > 0) {
			JDBCTemplate.commit(conn);
		} else {
			JDBCTemplate.rollback(conn);
		}
	...
	public List<Map<String, Object>> selectReply(HttpServletRequest req) {
		...
		return adminQnaDao.getMapList(conn, req);
	}
